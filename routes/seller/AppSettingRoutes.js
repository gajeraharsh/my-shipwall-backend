...
const {
  ...
  getStatesDropdown,
  getCitiesDropdown,
} = require("../../controllers/seller/AppSettingController");
...
} = require("../../validations/seller/AppSetting");

const router = express.Router();

router
  .route("/state")
  .post(validate(createStateValidation), createStateController);
router.route("/state").get(getStatesController);
router.route("/state/dropDown").get(getStatesDropdown);

router
  .route("/state/:id")
  .get(validate(getStateValidation), getStateByIdController);
router.put(
  "/state/:id",
...
router
  .route("/city")
  .post(validate(createCityValidation), createCityController);
router.route("/city").get(getCitiesController);
router.route("/city/dropDown").get(getCitiesDropdown);
router
  .route("/city/:id")
  .get(validate(getCityValidation), getCityByIdController);
...
router
  .route("/incentive")
  .post(validate(createIncentiveValidation), createIncentiveController);
router.route("/incentive").get(getIncentivesController);
router
  .route("/incentive/:id")
  .get(validate(getIncentiveValidation), getIncentiveByIdController);
router.put(
  "/incentive/:id",
  validate(updateIncentiveValidation),
  updateIncentiveController
);
router.delete(
  "/incentive/:id",
  validate(deleteIncentiveValidation),
  deleteIncentiveController
);
