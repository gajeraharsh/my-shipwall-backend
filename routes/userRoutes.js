...
const { getUserById } = require("../controllers/seller/saleUserController");
...
router.route("/forgotpassword").post(forgotPasswordSellerController);
  ...
  (req, res, next) => {
    if (req.files) {
      ...
      if (req.files.businessFrontPremisesPhoto) {
        req.body.businessFrontPremisesPhoto =
          req.files.businessFrontPremisesPhoto[0].originalname;
      }
      if (req.files.businessStockWithOwnerPhoto) {
        req.body.businessStockWithOwnerPhoto =
          req.files.businessStockWithOwnerPhoto[0].originalname;
      }
      if (req.files.ownerPhoto) {
        req.body.ownerPhoto = req.files.ownerPhoto[0].originalname;
      }
      if (req.files.visitingCardPhoto) {
        req.body.visitingCardPhoto =
          req.files.visitingCardPhoto[0].originalname;
      }
      if (req.files.gstCertificate) {
        req.body.gstCertificate = req.files.gstCertificate[0].originalname;
      }
      if (req.files.businessAddressProof) {
        req.body.businessAddressProof =
          req.files.businessAddressProof[0].originalname;
      }
    }
    next();
  },
...
router
  ...
  .put(verifyJWT, authorizeRoles("admin"), updateVerification);
