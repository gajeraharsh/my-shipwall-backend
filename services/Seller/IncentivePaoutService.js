...
const fetchIncentivePayouts = async (req) => {
  const {
    page = 1,
    limit = 10,
    search = "",
  ...
  if (state) {
    matchStage["salePersonInfo.permenentAddress.state._id"] =
      new mongoose.Types.ObjectId(state);
  }

  // Filter by city (user billing address city)
  if (city) {
    matchStage["salePersonInfo.permenentAddress.city._id"] =
...
const fetchIncentivePayoutReport = async (req) => {
  const {
    month,
    year,
    page = 1,
  ...
  // Handle state filter
  if (state) {
    userMatchConditions["salePersonInfo.permenentAddress.state"] =
      new mongoose.Types.ObjectId(state);
  }

  // Handle city filter
  if (city) {
    userMatchConditions["salePersonInfo.permenentAddress.city"] =
      new mongoose.Types.ObjectId(city);
  }
